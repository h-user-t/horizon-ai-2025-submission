...
import { Video, Users, Calendar as CalendarIcon, Clock, Plus, ArrowRight } from 'lucide-react';
...
export default function SchedulePage() {
	...
	useEffect(() => {
		const fetchData = async () => {
			try {
				...
				const q = query(sessionsRef, where('patientId', '==', currentUser.uid), where('sessionDate', '>=', now));
	...
	const handleBookAppointment = async () => {
		...
		try {
			...
			setSessions([
				...sessions,
				{ ...newSession, id: sessionRef.id, therapist: `Dr. ${therapists.find((t) => t.id === selectedTherapist)?.last_name}`, icon: Video },
			]);
			setOpen(false);
			...
			setSuccessMessage(
				`Appointment booked with Dr. ${
					therapists.find((t) => t.id === selectedTherapist)?.last_name
				} on ${sessionDate.toLocaleDateString()} at ${timeFormatted}.`
			);
